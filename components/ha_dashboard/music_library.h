...
struct QuickItem {
  ...
  std::string cover_url;     // resolved cover (300x300), fetched + decoded by the renderer
  bool has_children{false};  // podcast/audiobook -> drill into episodes/chapters before playing
};
...
class MusicLibraryBackend {
 public:
  ...
  // GET /api/v1/quick/{owner} -> the profile's favourites (cover grid).
  virtual void fetch_favorites(const std::string &owner, QuickItemsCallback cb) = 0;

  // Episodes/chapters of a podcast/audiobook (drill-down). uri = the parent item uri.
  virtual void fetch_children(const std::string &uri, QuickItemsCallback cb) = 0;
